	...
	def get_order(self, warehouse_id, filter_date):
		...
		if auth_token:
			...
			if branch_hid:
				...
				_logger.warning('Branch %s H_id not exist in branch mapping table, Please configure or fetch again from foodic', warehouse_id.name)
...
class FoodicsOrderProcess(models.Model):
	...
	def action_close_session(self, session_id):
		try:
			_logger.info("==call action_close_session== %s", session_id)
			session_id.action_pos_session_closing_control()
		except Exception as e:
			_logger.info("Session close fail %s", e)
			try:
				order_res = self.env['pos.order'].search([('session_id', '=', session_id.id)])
				for order_rec in order_res:
					for line_rec in order_rec.lines:
	...
	def create_delivery_line(self, product_obj, order_dic):
		...
		order_line_dic = {
			'qty': 1,
			'price_unit': round(order_dic['delivery_price'], 2),
			'price_subtotal': round(order_dic['delivery_price'], 2),
			'price_subtotal_incl': round(order_dic['delivery_price'], 2),
		}
		...
		if delivery_product_id:
			if not delivery_product_id.taxes_id:
				tax_ids = configuration_obj.excluded_taxes_id
				tax_li = []
				for tax_id in tax_ids:
					tax_li.append(tax_id.id)
	...
	def create_discount_line(self, product_obj, order_dic):
		...
		order_line_dic = {
			'qty': 1,
			'price_unit': round((order_dic['discount_amount'] * -1), 2),
			'price_subtotal': round((order_dic['discount_amount'] * -1), 2),
			'price_subtotal_incl': round((order_dic['discount_amount'] * -1), 2),
		}
		configuration_obj = self.env[
				'foodics.configuration'].search([], limit=1)
	...
	def create_line_for_modifiers(self, pro_map_id, line_data, product_obj):
		...
		for oprtion_data in line_data['options']:
			...
			if oprtion_data['relationship_data']:
				...
				addon_amount = round(oprtion_data['relationship_data']['original_price'], 2) * addon_qty
		...
		if branch_mapping_id:
			...
			if picking_type_id:
				...
				if pos_config_id:
					...
					raise Warning("PoS Config does not exist for branch %s" % branch_mapping_id.branch_id.name)
			else:
				raise Warning("Picking does not exist for branch %s" % branch_mapping_id.branch_id.name)
		else:
			foo_pos_order_res.write({'status': 'exceptions',
									 'fail_reason': 'Branch H_id' + order_dic['branch']['hid'] + 'does not exist in mapping table'})
		...
		if order_dic['payments']:
			for payment_data in order_dic['payments']:
				...
				if payment_mapping_id:
					for payment_method_name in session_id.config_id.payment_method_ids:
						if payment_method_name.name == payment_mapping_id.method_name:
	...
	def process_line_onchanges(self, order_id):
		...
		for line_id in order_id.lines:
			...
			line_id.write({
				...
				'price_subtotal': round(line_price_subtotal, 2),
				'price_subtotal_incl': round(line_price_subtotal_incl, 2),
			})
	...
	def check_adjustment_amount(self, order_id):
		...
		if adjustment_amount < 1.00 and adjustment_amount > -1.00:
			configuration_obj = self.env[
				'foodics.configuration'].search([], limit=1)
			...
			if adjustment_product_id:
				order_id.lines.create({
					...
					'qty': 1,
					'price_unit': round((float(adjustment_amount) * -1), 2),
	...
	def check_order_valid(self, foo_pos_order_res, order_dic):
		...
		order_mapping_obj = self.env['foodics.orders.mapping']
		# if order_dic['reference'] == 'QNWTB01C0143350':
